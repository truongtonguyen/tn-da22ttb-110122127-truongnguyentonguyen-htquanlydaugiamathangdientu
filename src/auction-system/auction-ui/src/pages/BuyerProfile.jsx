...
import {
  ShieldAlert, ShoppingBag, VolumeX, AlertTriangle,
  MessageCircle, Star, Lock, X, CircleDollarSign,
} from "lucide-react";

// Lý do dành riêng cho báo cáo NGƯỜI MUA — khác với người bán
// (bỏ WRONG_DESCRIPTION, FAKE_PRODUCT vì không áp dụng cho buyer)
const REASONS = [
  { key: "PAYMENT_NO_SHOW", icon: CircleDollarSign, label: "Thắng/mua ngay nhưng không thanh toán" },
  { key: "NO_RESPONSE",     icon: VolumeX, label: "Không phản hồi sau khi thắng" },
...
const BuyerProfile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await axiosClient.get(`/buyers/${id}`);
        setBuyer(res.data);
      } catch (err) {
        console.log(err);
      } finally {
  ...
  if (!buyer) return (
    <div style={styles.page}><Header />
      <p style={{ textAlign: "center", marginTop: 60 }}>Không tìm thấy người mua</p>
    </div>
  );

  const stats = [
    { label: "Số lượt đặt giá",      value: buyer.totalBids },
    { label: "Số lần thắng/mua",     value: buyer.totalOrders },
    { label: "Đã thanh toán",        value: buyer.paidOrders },
    { label: "Tỉ lệ thanh toán",     value: `${buyer.paymentRate}%`,
      color: buyer.paymentRate >= 70 ? "#2e7d32" : "#e53935" },
  ];
        ...
        <div style={styles.modalOverlay} onClick={() => setShowReport(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={{ margin: 0, fontSize: 17 }}><ShieldAlert size={18} style={{ verticalAlign: "middle", marginRight: 6 }} />Báo cáo người mua</h3>
      ...
      <div style={styles.container}>
        <div style={styles.hero}>
          ...
          <div style={styles.heroInfo}>
            <h1 style={styles.displayName}>{buyer.displayName}</h1>
            <div style={styles.username}>@{buyer.username}</div>
            <div style={{
              ...styles.creditBadge,
              color: creditColor(buyer.creditScore),
              backgroundColor: creditBg(buyer.creditScore),
            }}>
              <Star size={14} style={{ verticalAlign: "middle", marginRight: 4 }} />Điểm tín nhiệm: {buyer.creditScore}/100 — {buyer.creditLevel}
            </div>
          </div>
          {token && (
            <button
